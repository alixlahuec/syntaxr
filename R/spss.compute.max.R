...
spss.compute.max <- function(var1, var2, file = FALSE, ext = "txt", append = "new.") {
  created <- paste0(append, var2)
  output <- spss.format.max(var1, var2) %>%
              spss.format.compute(created, .)
  # Need to figure out if using a simple statement or a custom function for adding EXECUTE.
  # max_complete <- paste(max_format, "EXECUTE.")
  # Need to figure out how to organize output
